/*:
 **Target：understand recursive algorithms**
 
 Note：
 
 In the previous chapter,you must judge the parity of number
 
 There is a method called recursive algorithm, which makes it easier to finish Hanoi with any number of doughnuts.
	
 You can change the numberOfDoughtnuts,you can solve the Hanoi problem by the same code 
 
 Hint：use 'step through my code',observe how the recursive algorithm is performed
 */
//#-hidden-code
...
var doughtnutNumber: Int = /*#-editable-code*/6/*#-end-editable-code*/

// speed of doughnut
// the smaller number, move faster
var speed: TimeInterval = /*#-editable-code*/0.1/*#-end-editable-code*/
//#-hidden-code
    ...
    override public func viewDidAppear(_ animated: Bool) {
        ...
        if doughtnutNumber <= 0 || doughtnutNumber >= 7 {
            let alertController = UIAlertController(title: "doughtnutNumber错误", message: "甜甜圈数量只能在1-6之间", preferredStyle: .alert)
...
{
    //#-end-hidden-code
    // move n doughnuts from chocolatebar 'from' to chocolatebar 'to' with chocolatebar 'dependOn'
    playHanoiTower(n: doughtnutNumber, from: A, dependOn: B, to: C)
    //#-end-editable-code
    //#-hidden-code
...
func move(n: Int, from: ChocolateBarTag, to: ChocolateBarTag) {
    moveDoughtnut(from: from, to: to)
}

public func playHanoiTower(n: Int, from: ChocolateBarTag, dependOn: ChocolateBarTag, to: ChocolateBarTag) {
    
    // when n is equal to 1
    if n == 1 {
        
        // move one doughnuts from chocolatebar 'from' to chocolatebar 'to'
        move(n: 1, from: from, to: to)
        
    }else {
        
        // move the upper n-1 doughnuts from chocolatebar 'from' to chocolatebar 'dependOn' with chocolatebar 'dependOn'
        playHanoiTower(n: n - 1, from: from,dependOn: to, to: dependOn)
        
        // move No.n doughnuts from chocolatebar 'from' to chocolatebar 'to'
        move(n: n, from: from, to: to)
        
        // move the upper n-1 doughnuts from chocolatebar 'dependOn' to chocolatebar 'to' with chocolatebar 'from'
...
public extension GameViewController {
    ...
    func moveDoughtnut(from: ChocolateBarTag, to: ChocolateBarTag) {
        ...
        if from == A {
            ...
            let alertController = UIAlertController(title: "检查一下", message: "没有目标巧克力棒", preferredStyle: .alert)
            self.present(alertController, animated: true, completion: nil)
        }
        ...
        if(fromBarUp.last?.tag == 0) {
            let alertController = UIAlertController(title: "检查一下", message: "可以看看提示哦!", preferredStyle: .alert)
            self.present(alertController, animated: true, completion: nil)
            PlaygroundPage.current.assessmentStatus = .fail(hints: ["没有甜甜圈可以拿"], solution: nil)
        }else if(fromBarUp.last!.tag > toBarUp.last!.tag && toBarUp.last?.tag != 0) {
            let alertController = UIAlertController(title: "检查一下", message: "可以看看提示哦!", preferredStyle: .alert)
            self.present(alertController, animated: true, completion: nil)
            PlaygroundPage.current.assessmentStatus = .fail(hints: ["小甜甜圈上不能放大甜甜圈"], solution: nil)
        }else {
            print("移动")
    ...
    @objc public func successPass() {
        PlaygroundPage.current.assessmentStatus = .pass(message: "Is recursion very wonderful?\n\n [Going Further！](@next)")
    }
