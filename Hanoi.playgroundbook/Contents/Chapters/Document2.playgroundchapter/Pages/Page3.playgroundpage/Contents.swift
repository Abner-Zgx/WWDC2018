/*:
 **Target：No tips, use the pattern to achieve the goal**
 
 Note：
 * If you have an odd number of doughnuts, move the smallest doughnut from now to the next bar by the cyclic order of A➔C➔B➔A
 
 * If you have an even number of doughnuts, move the smallest doughnut from now to the next bar by the cyclic order of A➔B➔C➔A
 
 Hint：
 
 Change the speed of doughnut by modifying the code
 
 Change the number of doughnut by modifying the code
 */
//#-hidden-code
...
public class GameViewController: UIViewController, PlaygroundLiveViewSafeAreaContainer {
    ...
    var doughtnutNumber: Int = /*#-editable-code*/3/*#-end-editable-code*/
    ...
    override public func viewDidAppear(_ animated: Bool) {
        ...
        if doughtnutNumber <= 0 || doughtnutNumber >= 7 {
...
public extension GameViewController {
    ...
    public func setupUI(){
        ...
        //提示
        if (doughtnutNumber == 2 || doughtnutNumber == 4) {
            setupEvenHint()
        }else if (doughtnutNumber == 1 || doughtnutNumber == 3 || doughtnutNumber == 5) {
            setupOddHint()
        }else{
            //6层无提示
            hint66 = createHint(hint: hint66, text: "follow the tips on the left to try it yourself", alpha: 1.0)
        }
    ...
    //MARK:- 偶数提示
    public func setupEvenHint() {
        
        //线索1
        hint1 = createHint(hint: hint1, text: "move the smallest doughnut by the cyclic order of A➔B", alpha: 1.0)
        
        //线索2
        hint2 = createHint(hint: hint2, text: "move the movable doughnut which is the seceond smallest one", alpha: 0.0)
        
        //线索3
        hint3 = createHint(hint: hint3, text: "move the smallest doughnut by the cyclic order of B➔C", alpha: 0.0)
        
        //线索4
        hint4 = createHint(hint: hint4, text: "move the movable doughnut which is the seceond smallest one", alpha: 0.0)
        
        //线索5
        hint5 = createHint(hint: hint5, text: "move the smallest doughnut by the cyclic order of C➔A", alpha: 0.0)
        
        //线索6
        hint6 = createHint(hint: hint6, text: "move the movable doughnut which is the seceond smallest one", alpha: 0.0)
        
        //线索7
        hint7 = createHint(hint: hint7, text: "move the smallest doughnut by the cyclic order of A➔B", alpha: 0.0)
        
        //线索8
        hint8 = createHint(hint: hint8, text: "move the movable doughnut which is the seceond smallest one", alpha: 0.0)
        
        //线索9
        hint9 = createHint(hint: hint9, text: "move the smallest doughnut by the cyclic order of B➔C", alpha: 0.0)
        
        //线索10
        hint10 = createHint(hint: hint10, text: "move the movable doughnut which is the seceond smallest one", alpha: 0.0)
        
        //线索11
        hint11 = createHint(hint: hint11, text: "move the smallest doughnut by the cyclic order of C➔A", alpha: 0.0)
        
        //线索12
        hint12 = createHint(hint: hint12, text: "move the movable doughnut which is the seceond smallest one", alpha: 0.0)
        
        //线索13
        hint13 = createHint(hint: hint13, text: "move the smallest doughnut by the cyclic order of A➔B", alpha: 0.0)
        
        //线索14
        hint14 = createHint(hint: hint14, text: "move the movable doughnut which is the seceond smallest one", alpha: 0.0)
        
        //线索15
        hint15 = createHint(hint: hint15, text: "move the smallest doughnut by the cyclic order of B➔C", alpha: 0.0)
        
    }
    
    //MARK:- 奇数提示
    public func setupOddHint() {
...
public extension GameViewController {
    ...
    func moveDoughtnut(from: ChocolateBarTag, to: ChocolateBarTag) {
        ...
        if(fromBarUp.last?.tag == 0) {
            ...
            UIView.animate(withDuration: speed, delay: delayTime, options: .curveEaseInOut, animations: {
                ...
                if (self.doughtnutNumber == 1 || self.doughtnutNumber == 3 || self.doughtnutNumber == 5){
                    //奇数完成后跳出成功提示
                    self.ifLabelNeedOddAnimate(from: from, to: to)
                }else if (self.doughtnutNumber == 2 || self.doughtnutNumber == 4){
                    //偶数完成后跳出成功提示
                    self.ifLabelNeedEvenAnimate(from: from, to: to)
                }
    ...
    //MARK:- 奇数提示动画
    func ifLabelNeedOddAnimate(from: ChocolateBarTag, to: ChocolateBarTag) {
        progress += 1
